package com.vio.viaplaydemo.ui.views
...
fun ProductsGridScreen(
    ...
    Column(
        modifier = modifier
            .fillMaxSize()
            .background(ViaplayTheme.Colors.background),
    ) {
                ...
                LazyVerticalGrid(
                    ...
                    items(state.products) { product ->
                        VioProductCard(
                            ...
                            onAddToCart = { variant: live.vio.VioUI.Managers.Variant?, quantity: Int ->
...
private fun ProductsToolbar(title: String) {
    Row(
        ...
        IconButton(onClick = {}) {
            Icon(Icons.Filled.FilterList, contentDescription = null, tint = ViaplayTheme.Colors.textPrimary)
        }
        IconButton(onClick = {}) {
            Icon(Icons.Filled.GridView, contentDescription = null, tint = ViaplayTheme.Colors.primary)
        }
    }
}
...
private fun ProductsLoading() {
    Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
        androidx.compose.material3.CircularProgressIndicator(color = ViaplayTheme.Colors.primary)
    }
}
...
private fun ProductsError(message: String, onRetry: () -> Unit) {
    Column(
        ...
        Text("Noe gikk galt", style = ViaplayTheme.Typography.title, color = Color.Red)
        Text(message, style = ViaplayTheme.Typography.body, color = ViaplayTheme.Colors.textSecondary, modifier = Modifier.padding(top = 16.dp))
...
private fun ProductsEmpty() {
    Column(
        ...
        Text("Ingen produkter", style = ViaplayTheme.Typography.title, color = ViaplayTheme.Colors.textPrimary)
        Text(
