...
### Nuevos paquetes en `library/live.vio.VioUI/Components/compose`
...
- El módulo `library/live.vio.VioUI/Components/android` contiene los adaptadores de plataforma: `VioCoilImageLoader`, `CheckoutDeepLinkBus`, `KlarnaBridge`, `KlarnaNativeActivity`, `KlarnaWebActivity`, `PaymentSheetBridge`, el overlay Compose y los recursos (`ic_klarna`, layout nativo, network config).
- `VioCoilImageLoader` (`live.vio.VioUI.adapters.VioCoilImageLoader`) provee la implementación Coil del slot `VioImageLoader` para que cualquier app Compose pueda usarlo sin escribir código local.
