...
- Además del SDK puro, la carpeta `ReachuCore` replica la configuración del SDK Swift (temas, mercados, live show) para integrarse en demos que necesiten cargar `vio-config*.json` (`library/live.vio.VioCore/configuration/ConfigurationLoader.kt:1-200`).
